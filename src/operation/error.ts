...
export class INVALID_ED25519_PK extends OperationError {
  constructor(publicKey: string) {
    super({
      ...
      details:
        `The provided public key ${publicKey} is not a valid Stellar Ed25519 key. It must follow the strkey standard.`,
...
export class CANNOT_CONVERT_SPEND_OP extends OperationError {
  constructor(utxoPublicKey: UTXOPublicKey) {
    super({
      ...
      details:
        `The conversion of operation to condition failed because the operation is of type SPEND. This type cannot be used as a condition.`,
      data: { utxoPublicKey },
    });
  }
}
...
export class UNSUPPORTED_OP_TYPE_FOR_SCVAL_CONVERSION extends OperationError {
  constructor(opType: string) {
    super({
      ...
      details:
        `The operation type ${opType} is not supported for SCVal conversion. Only DEPOSIT, WITHDRAW, CREATE and SPEND types are supported.`,
      data: { opType },
    });
  }
}
...
export class OP_IS_NOT_CREATE extends OperationError {
  constructor(opType: string) {
    super({
      ...
      details:
        `The current operation could not be converted to ScVal as a CREATE operation because the type doesn't match.`,
      data: { opType },
    });
  }
}
...
export class OP_IS_NOT_SPEND extends OperationError {
  constructor(opType: string) {
    super({
      ...
      details:
        `The current operation could not be converted to ScVal as a SPEND operation because the type doesn't match.`,
      data: { opType },
    });
  }
}
...
export class OP_IS_NOT_DEPOSIT extends OperationError {
  constructor(opType: string) {
    super({
      ...
      details:
        `The current operation could not be converted to ScVal as a DEPOSIT operation because the type doesn't match.`,
      data: { opType },
    });
  }
}
export class OP_IS_NOT_WITHDRAW extends OperationError {
  constructor(opType: string) {
    super({
      ...
      details:
        `The current operation could not be converted to ScVal as a WITHDRAW operation because the type doesn't match.`,
      data: { opType },
    });
  }
}
...
export class OP_IS_NOT_SIGNABLE extends OperationError {
  constructor(opType: string, signType: string) {
    super({
      ...
      details:
        `The operation type ${opType} does not support signing with ${signType}.`,
      data: { opType },
    });
  }
}
...
export class OP_HAS_NO_CONDITIONS extends OperationError {
  constructor(utxoPublicKey: UTXOPublicKey) {
    super({
      ...
      message:
        `The SPEND operation for UTXO ${utxoPublicKey} has no conditions.`,
      details:
        `The operation of UTXO ${utxoPublicKey} cannot be signed because it has no conditions.`,
      data: { utxoPublicKey },
    });
  }
}
...
export class SIGNER_IS_NOT_DEPOSITOR extends OperationError {
  constructor(signerPk: string, depositorPk: string) {
    super({
      ...
      message:
        `Signer public key ${signerPk} does not match depositor public key ${depositorPk}.`,
      details:
        `The operation cannot be signed because the signer's public key does not match the depositor's public key defined in the operation.`,
...
export class OP_ALREADY_SIGNED extends OperationError {
  constructor(signatureType: string) {
    super({
      ...
      details:
        `The operation cannot be signed again because it is already signed with ${signatureType}.`,
      data: { signatureType },
    });
  }
}
...
export class INVALID_SCVAL_TYPE_FOR_OPERATION extends OperationError {
  constructor(expectedType: string, actualType: string) {
    super({
      ...
      details:
        `The SCVal type ${actualType} is not valid for conversion to the expected operation type ${expectedType}.`,
...
export class INVALID_SCVAL_VEC_FOR_OPERATION extends OperationError {
  constructor() {
    super({
      ...
      details:
        `The SCVal type is null is not a vector type required for operation conversion.`,
      data: {},
    });
  }
}
...
export class INVALID_SCVAL_VEC_LENGTH_FOR_OPERATION extends OperationError {
  constructor(opType: string, expectedLength: number, actualLength: number) {
    super({
      ...
      details:
        `The SCVal vector length ${actualLength} does not match the expected length ${expectedLength} for operation type ${opType}.`,
...
export class INVALID_SCVAL_VEC_FOR_CONDITIONS extends OperationError {
  constructor(utxoOrPk: UTXOPublicKey | Ed25519PublicKey) {
    super({
      ...
      message:
        `Invalid SCVal vector for conditions in op with address ${utxoOrPk}.`,
      details:
        `The SCVal vector for conditions in op with address ${utxoOrPk} is not valid.`,
      data: { utxoOrPk },
    });
  }
}
...
export class INVALID_SCVAL_VEC_FOR_CONDITION extends OperationError {
  constructor(utxoOrPk: UTXOPublicKey | Ed25519PublicKey, type: string) {
    super({
      ...
      message:
        `Invalid SCVal vector for condition in op with address ${utxoOrPk}.`,
      details:
        `The SCVal vector for a condition in op with address ${utxoOrPk} is not valid. A type of ${type} was provided instead of an ScVec.`,
      data: { utxoOrPk, type },
    });
  }
}
